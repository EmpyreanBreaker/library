  ...
  changeBookStatus(bookIndex) {
    for (let i = 0; i < this.library.length; i++) {
      if (this.library[i].id === bookIndex) {
        this.library[i].status === "Read"
          ? (this.library[i].status = "Unread")
          : (this.library[i].status = "Read");
        break;
      }
    }
...
myLibrary.addBookToLibrary(
  new Book("To Kill a Mockingbird", "Harper Lee", "A Novel", "Read"),
);

myLibrary.addBookToLibrary(
  new Book("1984", "George Orwell", "A Dystopian Novel", "Unread"),
);
...
myLibrary.addBookToLibrary(
  new Book(
    "Pride and Prejudice",
    "Jane Austen",
    "A Story of Love and Misunderstanding",
    "Unread",
  ),
);
...
myLibrary.addBookToLibrary(
  new Book(
    "Foundation",
    "Isaac Asimov",
    "The Saga of the Psychohistorians",
    "Unread",
  ),
);
...
myLibrary.addBookToLibrary(
  new Book("Kindred", "Octavia E. Butler", "A Novel", "Read"),
);

myLibrary.addBookToLibrary(
  new Book("Beloved", "Toni Morrison", "A Novel", "Read"),
);

myLibrary.addBookToLibrary(
  new Book("Mistborn", "Brandon Sanderson", "The Final Empire", "Unread"),
);
...
myLibrary.addBookToLibrary(
  new Book(
    "Dune",
    "Frank Herbert",
    "A Heroic Saga on the Desert Planet Arrakis",
    "Read",
  ),
);
...
const cancelButton = document.querySelector(".dialog__button-cancel");
